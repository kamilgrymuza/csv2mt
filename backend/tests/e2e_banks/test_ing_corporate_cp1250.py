"""
E2E Test: ING Corporate Bank Statement

This test validates the complete pipeline:
1. Read CSV file (with potential encoding issues like iso-8859-2)
2. Parse using current implementation (Claude AI or format detection)
...
class TestINGCorporateE2E:
    """End-to-end test for ING Corporate bank statement conversion"""

    @pytest.fixture
    def test_dir(self):
        """Get the test directory for this bank"""
        return Path(__file__).parent / "ing_corporate_cp1250"
    ...
    def test_ing_corporate_csv_to_mt940(self, input_csv, expected_mt940, output_mt940):
        """
        Test complete CSV → MT940 conversion pipeline for ING Corporate
        """
        # Step 1: Parse CSV file
        print("\n" + "="*80)
        print("E2E TEST: ING Corporate CSV → MT940")
        print("="*80)
        print(f"\nStep 1: Parsing input CSV from {input_csv}")

        parser = ClaudeDocumentParser()

        # Read CSV with encoding detection (ING uses iso-8859-2 or similar)
        ...
        assert len(result['transactions']) > 0, "No transactions parsed from CSV"
