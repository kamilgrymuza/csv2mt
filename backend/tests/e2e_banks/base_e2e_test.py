"""
Base class for E2E bank statement conversion tests.

This module provides common functionality for all bank-specific E2E tests,
including fixtures, comparison logic, and helper methods.
"""
...
class BaseE2ETest:
    """
    Base class for end-to-end bank statement conversion tests.

    Subclasses should:
    1. Set the `bank_name` class attribute to identify the test
    2. Set the `test_folder_name` class attribute to match the subdirectory name
    3. Implement a test method that calls `run_conversion_test()`
    """

    # Subclasses must override these
    bank_name = None  # e.g., "Santander Individual"
    test_folder_name = None  # e.g., "santander_individual"

    @pytest.fixture
    def test_dir(self):
        """Get the test directory for this bank"""
        if not self.test_folder_name:
            raise NotImplementedError("Subclass must set test_folder_name")
        return Path(__file__).parent / self.test_folder_name

    @pytest.fixture
    def input_file(self, test_dir):
        """Path to input file (CSV or PDF)"""
        # Try common input file names
        for name in ["input.csv", "input.pdf", "input.xlsx"]:
            path = test_dir / name
            if path.exists():
                return path
        raise FileNotFoundError(f"No input file found in {test_dir}")
    ...
    def run_conversion_test(self, input_file, expected_mt940, output_mt940, allow_empty=False, expect_exception=None):
        ...
        Args:
            input_file: Path to input file (CSV/PDF/XLSX)
        ...
        print("\n" + "="*80)
        print(f"E2E TEST: {bank_display_name} → MT940")
        print("="*80)
        print(f"\nStep 1: Parsing input file from {input_file}")

        # Use disable_cache=True in tests to avoid cached responses affecting test determinism
        parser = ClaudeDocumentParser(disable_cache=True)

        # Read input file
        with open(input_file, 'rb') as f:
            result = parser.parse_document(f, input_file.name)
        ...
        if allow_empty and len(generated_statements) == 0 and len(expected_statements) == 0:
    ...
    def _compare_statements(self, generated, expected, gen_file, exp_file):
        ...
        # Compare transaction count - FAIL IMMEDIATELY if mismatch
        ...
        if gen_txn_count != exp_txn_count:
            ...
            assert False, f"Transaction count mismatch: generated {gen_txn_count}, expected {exp_txn_count}"
        else:
